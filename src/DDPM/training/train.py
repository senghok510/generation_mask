...
from DDPM.data.dataset import FaceMaskDataset
from DDPM.core.diffusion import DiffusionSchedule
from DDPM.core.model import ConditionalUNet
from DDPM.core.utils import choose_device, ensure_dir, save_json, seed_everything
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save-dir", default="outputs/DDPM_base", help="Checkpoint and metadata directory.")
...
def update_ema(ema_model: nn.Module, model: nn.Module, decay: float) -> None:
    with torch.no_grad():
        ...
        for name, buffer in dict(model.named_buffers()).items():
...
def compute_loss(
    ...
    condition = torch.cat([masked_clean, mask], dim=1)
    if cfg_dropout > 0.0:
    ...
    mask_exp  = mask.expand_as(pred_noise)
    mask_area = mask_exp.sum().clamp(min=1.0)
    loss = ((pred_noise - noise) ** 2 * mask_exp).sum() / mask_area
    return loss
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            if batch_idx % 20 == 0 or batch_idx == len(train_loader):
                ...
                print(f"epoch={epoch} batch={batch_idx}/{len(train_loader)} loss={running:.4f} lr={lr_now:.2e}")

        train_loss = total_loss / max(len(train_loader), 1)
        val_loss   = evaluate(ema_model, diffusion, val_loader, cfg_dropout=0.0, device=device)
        print(
            f"epoch={epoch} train_loss={train_loss:.4f}"
            + (f" val_loss={val_loss:.4f}" if val_loss is not None else "")
        )

        save_checkpoint(output_dir / "checkpoint_last.pt", model, ema_model, optimizer, scheduler, epoch, val_loss, args)
        if val_loss is None:
            save_checkpoint(output_dir / "checkpoint_best.pt", model, ema_model, optimizer, scheduler, epoch, val_loss, args)
