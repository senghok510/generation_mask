...
class SimpleResidualBlock(nn.Module):
    """Residual block without time conditioning (for segmentation)."""
...
class SimpleStage(nn.Module):
    """Sequential stage of SimpleResidualBlocks and AttentionBlocks."""
...
class MaskPredictorUNet(nn.Module):
    """Segmentation U-Net: clean face (3ch) -> binary mask logits (1ch)."""

    def __init__(self, input_channels: int = 3, base_channels: int = 64, dropout: float = 0.0) -> None:
        super().__init__()
        self.stem = nn.Conv2d(input_channels, base_channels, kernel_size=3, padding=1)

        self.enc1 = SimpleStage(
            SimpleResidualBlock(base_channels, base_channels, dropout),
            SimpleResidualBlock(base_channels, base_channels, dropout),
        )
        self.down1 = Downsample(base_channels)

        self.enc2 = SimpleStage(
            SimpleResidualBlock(base_channels, base_channels * 2, dropout),
            SimpleResidualBlock(base_channels * 2, base_channels * 2, dropout),
        )
        self.down2 = Downsample(base_channels * 2)

        self.enc3 = SimpleStage(
            SimpleResidualBlock(base_channels * 2, base_channels * 4, dropout),
            AttentionBlock(base_channels * 4),
            SimpleResidualBlock(base_channels * 4, base_channels * 4, dropout),
        )
        self.down3 = Downsample(base_channels * 4)

        self.mid = SimpleStage(
            SimpleResidualBlock(base_channels * 4, base_channels * 8, dropout),
            AttentionBlock(base_channels * 8),
            SimpleResidualBlock(base_channels * 8, base_channels * 4, dropout),
        )

        self.up3 = Upsample(base_channels * 4)
        self.dec3 = SimpleStage(
            SimpleResidualBlock(base_channels * 8, base_channels * 4, dropout),
            AttentionBlock(base_channels * 4),
            SimpleResidualBlock(base_channels * 4, base_channels * 2, dropout),
        )

        self.up2 = Upsample(base_channels * 2)
        self.dec2 = SimpleStage(
            SimpleResidualBlock(base_channels * 4, base_channels * 2, dropout),
            SimpleResidualBlock(base_channels * 2, base_channels, dropout),
        )

        self.up1 = Upsample(base_channels)
        self.dec1 = SimpleStage(
            SimpleResidualBlock(base_channels * 2, base_channels, dropout),
            SimpleResidualBlock(base_channels, base_channels, dropout),
        )
